...
TSP viajero(ciudades);
float vertices[ciudades * 3];//Para las 3 coordenadas, x2 por los indices
int generaciones = 3000;

int main() {


	viajero.elegirCiudadInicial();


	viajero.getCoordsOpenGL(vertices);
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(vertices), vertices, GL_STATIC_DRAW);//Copia la data al buffer
	...
	float colorCity[] = {0.96f,0.43f,0.57f };
	float colorLine[] = { 0.55f,0.55f,0.67f };
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods) {
	...
	if (glfwGetKey(window, GLFW_KEY_N) == GLFW_PRESS) {
		if (generaciones) {
			viajero.generarNuevaPoblacion();
			viajero.getCoordsOpenGLBest(vertices);
			glBindVertexArray(VAO);
			glBindBuffer(GL_ARRAY_BUFFER, VBO);
			glBufferData(GL_ARRAY_BUFFER, sizeof(vertices), vertices, GL_DYNAMIC_DRAW);//Copia la data al buffer
			generaciones--;
